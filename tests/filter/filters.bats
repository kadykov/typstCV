...
@test "filter/typst-cv: handles location attribute" {
  ...
  assert_output --partial 'side: company-location()[City \ Country]'
}
...
@test "filter/typst-cv: handles date attribute" {
  ...
  run eval "$cmd"
  assert_success
  assert_output --partial 'side: event-date()[2023 - Present]'
}
...
@test "filter/typst-cv: handles header with both date and location (processes date)" {
  ...
  assert_output --partial 'side: event-date()[Date First])'
